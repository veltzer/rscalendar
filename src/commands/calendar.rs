...
pub async fn cmd_calendar_create(
...
pub async fn cmd_calendar_delete(
    client: &GoogleCalendarClient,
    name: &str,
    config: &Config,
    out: &OutputOptions,
    ...
    let confirmed = crate::helpers::prompt_confirm(&format!(
...
pub async fn cmd_calendar_rename(
    client: &GoogleCalendarClient,
    name: &str,
    new_name: &str,
    config: &Config,
    ...
    if !out.quiet {
        println!("Renamed calendar '{name}' to '{new_name}'.");
    }
    Ok(())
}

pub async fn cmd_calendar_clear(
    client: &GoogleCalendarClient,
    args: &CalendarClearArgs,
    config: &Config,
    out: &OutputOptions,
) -> Result<()> {
...
pub async fn cmd_calendar_copy(
    client: &GoogleCalendarClient,
    args: &CalendarCopyArgs,
    out: &OutputOptions,
) -> Result<()> {
    let calendars = client.list_calendars().await?;
    ...
    for event in &events {
        ...
        if args.dry_run {
            if !out.quiet {
                println!("[dry-run] would copy: {summary} ({start})");
            }
            copied += 1;
        } else {
    ...
    if !out.quiet {
        if args.dry_run {
            ...
            println!(
                "\nDone. {copied} event(s) copied to '{}', {skipped} skipped.",
                args.target
            );
        }
    }
